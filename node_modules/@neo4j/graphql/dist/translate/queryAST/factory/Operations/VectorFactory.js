...
class VectorFactory {
    ...
    createVectorOperation(entity, resolveTree, context) {
        ...
        const filteredResolveTree = (0, find_fields_by_name_in_fields_by_type_name_field_1.findFieldsByNameInFieldsByTypeNameField)(vectorResultField, entity.operations.rootTypeFieldNames.connection)[0];
        // Adds the args to the nested resolve tree for vector
        filteredResolveTree.args = resolveTree.args;
        const connectionFields = (0, get_fields_by_type_name_1.getFieldsByTypeName)(filteredResolveTree, entity.operations.vectorTypeNames.connection);
        const filteredResolveTreeEdges = (0, find_fields_by_name_in_fields_by_type_name_field_1.findFieldsByNameInFieldsByTypeNameField)(connectionFields, "edges");
    ...
    addScoreSort(operation, resolveTree, context) {
        const sortArguments = (resolveTree.args.sort ?? []);
        for (const sortArgument of sortArguments) {
            if (sortArgument[constants_1.SCORE_FIELD] && context?.vector) {
                const scoreSort = new ScoreSort_1.ScoreSort({
                    scoreVariable: context.vector.scoreVariable,
                    direction: sortArgument[constants_1.SCORE_FIELD],
                });
